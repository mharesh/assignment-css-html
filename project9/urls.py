"""
URL configuration for project9 project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from app.views import *

urlpatterns = [
    path('admin/', admin.site.urls),
    path('timetable/',timetable,name='timetable'),
    path('institute/',institute,name='institute'),
    path('drink/',drink,name='drink'),
    path('job/',job,name='job'),
    path('farmer/',farmer,name='farmer'),
    path('farmerdetails/',farmerdetails,name='farmerdetails'),
    path('matrimonial/',matrimonial,name='matrimonial'),
    path('mocktest/',mocktest,name='mocktest'),
    path('bootstrap/',bootstrap,name='bootstrap'),
    path('form/',form,name='form'),
    path('kmcustomform/',kmcustomform,name='kmcustomform'),
    path('studenttable/',studenttable,name='studenttable'),
    path('animations/',animations,name='animations'),
    path('wrapper/',wrapper,name='wrapper'),
    path('new/',new,name='new'),
]
